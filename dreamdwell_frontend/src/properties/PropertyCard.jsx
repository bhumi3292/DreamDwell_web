...
export default function PropertyCard({ property, currentUserId, onUpdate, onDelete, isDeleting }) {
    ...
    // --- CRUCIAL LOGS FOR DEBUGGING OWNERSHIP ---
    console.log(`Property: ${property.title} (ID: ${property._id})`);
    console.log(`  Property Landlord ID: ${property.landlord?._id}`);
    console.log(`  Current User ID (from AuthContext): ${currentUserId}`);
    console.log(`  Is Owner? ${currentUserId && property.landlord?._id === currentUserId}`);
    console.log("-----------------------------------------");
    // --- END CRUCIAL LOGS ---
    ...
    return (
        <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition overflow-hidden cursor-pointer relative" onClick={handleCardClick}>
            ...
            <div className="p-4 flex flex-col">
                <h3 className="text-xl font-bold text-[#002B5B] mb-1">{property.title}</h3>
                ...
                <p className="text-sm text-gray-600 my-2 line-clamp-3">{property.description}</p>
