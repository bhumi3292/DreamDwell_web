...
import { useParams, useNavigate } from 'react-router-dom'; // useParams to get token from URL
// <<<--- USE THE NEW HOOK FOR RESETTING PASSWORD WITH TOKEN
import { useResetPassword } from '../hooks/useAuthHooks';

function ResetPasswordWithToken() {
    const { token } = useParams(); // Get the token from the URL (e.g., /reset-password/YOUR_TOKEN)
    const navigate = useNavigate();

    const { mutate: resetPasswordMutation, isLoading: isResetting } = useResetPassword();

    // Basic check: If no token is present in the URL, redirect to forgot password
    useEffect(() => {
        if (!token) {
